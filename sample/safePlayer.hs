...
safeDecision (Info.PSI ste reTi (pos,vel) oSta obPo) = ask >>= \(Info.II _ _ width length vl) ->
              return $ head $ filter (adjastVel obPo pos vel width vl length) $ filter (adjastVelHorizontal pos vel width) $ filter (isNone obPo pos vel width vl length) $ filter (isInCource pos vel width) $ makeAccelerationCandidates vel


adjastVelHorizontal :: Info.Pos -> Info.Velocity -> Int -> (Int,Int) -> Bool
adjastVelHorizontal (px,_) (vx,_) width (acx,_) 
  | 2 >= px && vx == -2 = 0 < acx
...
-- adjastObPoVel :: Info.ObstaclePoints -> Info.Pos -> Info.Velocity -> Int -> Int -> (Int,Int) -> Bool
-- adjastObPoVel obPo (px,py) (vx,vy) width vl (acx,acy) 
--       | vx == 2 && vy == 2 = (obPo !! (estimatedOPindex+1)) /= Info.Obstacle || (obPo !! (estimatedOPindex+1+width)) /= Info.Obstacle || 
--         (obPo !! (estimatedOPindex+1-width)) /= Info.Obstacle || (obPo !! (estimatedOPindex+width)) /= Info.Obstacle || (obPo !! (estimatedOPindex+width+1)) /= Info.Obstacle || 
--         (obPo !! (estimatedOPindex+width-1)) /= Info.Obstacle
--       | vx == 2 = (obPo !! (estimatedOPindex+1)) /= Info.Obstacle || (obPo !! (estimatedOPindex+1+width)) /= Info.Obstacle || (obPo !! (estimatedOPindex+1-width)) /= Info.Obstacle
--       | vy == 2 = (obPo !! (estimatedOPindex+width)) /= Info.Obstacle || (obPo !! (estimatedOPindex+width+1)) /= Info.Obstacle || (obPo !! (estimatedOPindex+width-1)) /= Info.Obstacle
--       | vx == -2 && vy == -2 = (obPo !! (estimatedOPindex-1)) /= Info.Obstacle || (obPo !! (estimatedOPindex-1+width)) /= Info.Obstacle || 
--         (obPo !! (estimatedOPindex-1-width)) /= Info.Obstacle || (obPo !! (estimatedOPindex-width)) /= Info.Obstacle || 
--         (obPo !! (estimatedOPindex-width+1)) /= Info.Obstacle || (obPo !! (estimatedOPindex-width-1)) /= Info.Obstacle
--       | vx == -2 = (obPo !! (estimatedOPindex-1)) /= Info.Obstacle || (obPo !! (estimatedOPindex-1+width)) /= Info.Obstacle || (obPo !! (estimatedOPindex-1-width)) /= Info.Obstacle
--       | vy == -2 = (obPo !! (estimatedOPindex-width)) /= Info.Obstacle || (obPo !! (estimatedOPindex-width+1)) /= Info.Obstacle || 
--         (obPo !! (estimatedOPindex-width-1)) /= Info.Obstacle
--       | otherwise = True
--           where
...
isNone obPo (px,py) (vx,vy) width vl length (acx,acy) = if vl < (vy+acy) then False else (if length <= py+vy+acy then True else (Info.None == (obPo !! estimatedOPindex ) && (Info.None == obPo !! (estimatedOPindex+1) || (Info.None == obPo !! (estimatedOPindex-width))) && (if (vx+acx == 2) && (vy+acy == 2) then Info.None == obPo !! (estimatedOPindex-width-1) else True)))
                           where 
                                 estimatedOPindex = ( (vl * width + px) + (width * (vy+acy)) + (vx+acx) ) -- (vl * width + px) is present player's position
